module BBCodeizer  
  class << self
    ...
    Tags = {
      ...
      :end_quote             => [ /\[\/quote\](\s*)/i,                   '</blockquote>' ],
      ...
      :size                  => [ /\[size=(50|100|200)\](.+?)\[\/size\]/mi, '<span style="font-size: \1%">\2</span>' ],
      :color                 => [ /\[color=([^;]+?)\](.+?)\[\/color\]/mi,   '<span style="color: \1">\2</span>' ],
      :uncomplete            => [ /\[(\/?)(b|i|u|email|email=|url|url=|img|size|color)(\]| )/mi, '' ]
    }
    ...
    TagList = [ :bold, :italic, :underline, :email_with_name, :email_sans_name, 
                :url_with_title, :url_sans_title, :image, :size, :color, :code, :quote, :uncomplete ]
