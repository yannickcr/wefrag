...
class My::SessionsControllerTest < ActionController::TestCase
  def setup
  ...
  test 'on GET to :show' do
    ...
    assert_redirected_to new_my_session_path
  end
  ...
  test 'on GET to :new' do
    ...
    assert_select 'form[action=?] input', my_session_path do
  ...
  test 'on GET to :new as javascript' do
    ...
    assert_tag :tag => 'form', :attributes => { :action => my_session_path }
