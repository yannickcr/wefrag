...
class OpenidDecideWithIdentityControllerTest < ActionController::TestCase
  ...
  def setup
    ...
    @openid_params = {
      ...
      'openid.identity'   => @user.openid_identity,
  ...
  test 'on GET to :decide' do
    ...
    assert_select 'form[action=?] input', openid_decide_path(:req => encode(@openid_params)) do
      assert_select '[name=?]', 'login', false
  ...
  test 'on GET to :decide with authenticated user' do
    ...
    assert_select 'form[action=?] input', openid_decide_path(:req => encode(@openid_params))
  end

  test 'on GET to :decide with different authenticated user' do
    get :decide, { :req => encode(@openid_params) }, { :user_id => @other_user.id }
  ...
  test 'on POST to :decide' do
    assert_difference 'UserOpenidTrust.count' do
      post :decide, { :req => encode(@openid_params), :password => 'root' }
    end
  ...
  test 'on POST refuse to :decide' do
    assert_no_difference 'UserOpenidTrust.count' do
      post :decide, { :refuse => '', :req => encode(@openid_params), :password => 'root' }
    end
  ...
  test 'on POST refuse to :decide with another authenticated user' do
    assert_no_difference 'UserOpenidTrust.count' do
      post :decide, { :refuse => '', :req => encode(@openid_params) }, { :user_id => @other_user.id }
    end
